-- Enable RLS on tables that were missing it (idempotent — ENABLE is a no-op if already enabled)
...
-- Wrapped in DO blocks so re-running the migration (e.g. after a manual Supabase apply)
