...
-- CreateIndex
CREATE UNIQUE INDEX IF NOT EXISTS "organization_emission_factors_organization_id_scope_emission_ca_key" ON "organization_emission_factors"("organization_id", "scope", "emission_category_id", "activity_type", "geography_country", "geography_region", "input_unit", "version");

CREATE INDEX IF NOT EXISTS "organization_emission_factors_organization_id_scope_emission_ca_idx" ON "organization_emission_factors"("organization_id", "scope", "emission_category_id");

CREATE INDEX IF NOT EXISTS "organization_emission_factors_organization_id_effective_start__idx" ON "organization_emission_factors"("organization_id", "effective_start_date", "effective_end_date");

CREATE INDEX IF NOT EXISTS "supplier_anomalies_organization_id_anomaly_severity_idx" ON "supplier_anomalies"("organization_id", "anomaly_severity");

CREATE INDEX IF NOT EXISTS "supplier_anomalies_organization_id_detected_at_idx" ON "supplier_anomalies"("organization_id", "detected_at");

CREATE INDEX IF NOT EXISTS "supplier_anomalies_supplier_data_request_id_idx" ON "supplier_anomalies"("supplier_data_request_id");

CREATE UNIQUE INDEX IF NOT EXISTS "zapier_integrations_zapier_custom_id_key" ON "zapier_integrations"("zapier_custom_id");
