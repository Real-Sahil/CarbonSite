...
function useCanvasAnimation({
  ...
  const stateRef = useRef<CanvasState>({
    ...
    isVisible: true, isPageVisible: true,
    animationId: 0, frameCount: 0,
  });

  const onDrawRef = useRef(onDraw); onDrawRef.current = onDraw;
  const onSetupRef = useRef(onSetup); onSetupRef.current = onSetup;
  ...
  useEffect(() => {
    ...
    const setup = () => {
      ...
      st.width = rect.width; st.height = rect.height; st.dpr = dpr;
    ...
    const stop = () => {
      if (st.animationId) { cancelAnimationFrame(st.animationId); st.animationId = 0; }
    };

    let cancelIdle: (() => void) | undefined;
    if (deferStart) {
      const startNow = () => { canStart = true; cancelIdle = undefined; start(); };
    ...
    const onResizeEv = () => {
      ...
      resizeTimer = setTimeout(() => { setup(); onResizeRef.current?.(ctx, st); }, resizeDebounce);
    };
    const onPageVis = () => {
    ...
    return () => {
      stop(); cancelIdle?.(); clearTimeout(resizeTimer);
      io.disconnect();
...
export default function WaveArcs({
  ...
  const { containerRef, canvasRef, stateRef } = useCanvasAnimation({
    ...
    onDraw: (e, t) => {
      ...
      for (let k = 0; k < x; k++) {
        ...
        if (a > 499999.5) {
          e.beginPath(); e.moveTo(-h, -i / 2); e.lineTo(h, -i / 2); e.stroke(); continue;
        }
        const c2 = Math.acos(Math.min(1, (h + 50) / a));
        const segTotal = Math.max(Math.ceil(a / 120), 200);
        const spans: [number, number][] = [[c2, Math.PI - c2], [Math.PI + c2, TWO_PI - c2]];
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      if (rafId) return;
      rafId = requestAnimationFrame(() => { rect = container.getBoundingClientRect(); rafId = 0; });
    };
