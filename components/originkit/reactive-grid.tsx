...
export default function ReactiveGrid({
  ...
  const propsRef = useRef({ shape, fill, strokeWidth, particleColor, backgroundColor, maxSize, minSize, gap, influence });
  const currentRef = useRef<Float32Array>(new Float32Array(0));

  useEffect(() => {
    propsRef.current = { shape, fill, strokeWidth, particleColor, backgroundColor, maxSize, minSize, gap, influence };
  }, [shape, fill, strokeWidth, particleColor, backgroundColor, maxSize, minSize, gap, influence]);
  ...
  useEffect(() => {
    ...
    const buildPath = (cx: number, cy: number, s: number, shp: Shape) => {
      ...
      ctx.beginPath();
      switch (shp) {
        case "circle": ctx.arc(cx, cy, half, 0, Math.PI * 2); break;
        case "rounded": {
        ...
        case "triangle":
          ctx.moveTo(cx, cy - half); ctx.lineTo(cx + half, cy + half); ctx.lineTo(cx - half, cy + half); ctx.closePath(); break;
        case "diamond":
          ctx.moveTo(cx, cy - half); ctx.lineTo(cx + half, cy); ctx.lineTo(cx, cy + half); ctx.lineTo(cx - half, cy); ctx.closePath(); break;
        case "hexagon":
          for (let k = 0; k < 6; k++) {
            const a = ((-90 + 60 * k) * Math.PI) / 180;
            k === 0 ? ctx.moveTo(cx + half * Math.cos(a), cy + half * Math.sin(a))
        ...
        default: ctx.rect(cx - half, cy - half, s, s);
      }
    };
    ...
    const draw = () => {
      ...
      if (currentRef.current.length !== count) currentRef.current = new Float32Array(count).fill(p.minSize);
      ...
      for (let j = 0; j < rows; j++) {
        for (let i = 0; i < cols; i++) {
          const idx = j * cols + i;
          const cx = offX + i * cell, cy = offY + j * cell;
          let infl = 0;
          if (mouse) {
  ...
  return (
    <div
      ref={containerRef}
      className={className}
      style={{ position: "relative", width: "100%", height: "100%", overflow: "hidden", backgroundColor, ...style }}
    >
      <canvas
        ref={canvasRef}
