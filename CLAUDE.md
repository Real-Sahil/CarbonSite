...
- **Report types:** only the bid carbon pack, transition plan, ESRS E3/E5 and ecology reports render their own HTML layout. Every other emissions report type (SECR, `ppn_006_crp`, GHG Protocol, CDP, ESRS E1, National TOMs) returns `pdfkitData` from its handler in `lib/reports/registry.ts`, and `lib/reports/worker.ts` then renders the generic PDFKit report. Do not advertise those formats as distinct documents until that changes.
