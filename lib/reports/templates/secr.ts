...
export type SecrData = {
  ...
  /// Records left out of the energy figures because their quantity has no kWh equivalent here.
  energyRecordsNotConverted?: number;
  // GHG emissions (tCO2e)
  scope1Tonnes: number;
  scope2Tonnes: number;
  totalTonnes: number;
...
export function renderSecrHtml(d: SecrData): string {
  ...
  const emissionsDonut = svgDonut(
    [
      ...
      { label: "Scope 2 energy", value: d.scope2Tonnes * 1000, scope: 2 },
    ].filter((s) => s.value > 0),
  ...
  const energyBars = svgHBars(
    ...
    { unit: "", formatValue: (v) => Math.round(v / 1000).toLocaleString("en-GB") + " MWh" },
  );
...
<section>
  ...
  <p style="font-size:9pt;color:#6b7280;">Energy is taken from the activity records in this snapshot: kWh as recorded, and fuel in litres converted at DESNZ gross calorific values.${d.energyRecordsNotConverted ? ` ${d.energyRecordsNotConverted.toLocaleString("en-GB")} fuel or energy record${d.energyRecordsNotConverted === 1 ? " is" : "s are"} recorded in units with no kWh equivalent and ${d.energyRecordsNotConverted === 1 ? "is" : "are"} not included in the energy figures.` : ""}</p>
</section>
...
<section>
  ...
  <div class="intensity-box">
    ...
    <div style="font-size:9pt;color:#065f46;margin-top:6px;">Denominator: ${esc(d.intensityDenominator)}</div>`
      : `<div style="font-size:10pt;color:#065f46;">No intensity denominator was given for this report. SECR requires at least one intensity ratio: add the denominator (for example employees or turnover) under SECR intensity metrics when generating the report.</div>`}
  </div>
</section>
...
<div class="footer">
  ...
  Generated by MetricOra · Snapshot v${d.snapshotVersion} · Published ${now} by ${esc(d.publishedBy)} · ${esc(d.factorLibrary)} · ${esc(d.methodology)}
</div>
</body></html>`;
}
