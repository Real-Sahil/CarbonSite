...
export function CreateContractForm({ orgId }: { orgId: string }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-[14px] border border-[#e5e7eb] p-[21px] flex flex-col gap-4">
      <p className="text-sm font-normal text-[#0f3e17] tracking-[-0.42px]">New contract</p>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <div className="flex flex-col gap-1.5">
          <Label htmlFor="contract-name" className="text-xs text-[#222222] tracking-[-0.36px]">Name <span aria-hidden="true" className="text-red-500">*</span></Label>
          <Input id="contract-name" name="name" required placeholder="Contract name" className="h-9 text-sm" />
        </div>
        <div className="flex flex-col gap-1.5">
          <Label htmlFor="contract-client" className="text-xs text-[#222222] tracking-[-0.36px]">Client name</Label>
          <Input id="contract-client" name="clientName" placeholder="Client organisation" className="h-9 text-sm" />
        </div>
        <div className="flex flex-col gap-1.5">
          <Label htmlFor="contract-ref" className="text-xs text-[#222222] tracking-[-0.36px]">Contract reference</Label>
          <Input id="contract-ref" name="contractReference" placeholder="REF-001" className="h-9 text-sm" />
        </div>
        <div className="flex flex-col gap-1.5">
          <Label htmlFor="contract-status" className="text-xs text-[#222222] tracking-[-0.36px]">Status</Label>
          <select
            id="contract-status"
            name="status"
            defaultValue="active"
        ...
        <div className="flex flex-col gap-1.5">
          <Label htmlFor="contract-start" className="text-xs text-[#222222] tracking-[-0.36px]">Start date</Label>
          <Input id="contract-start" name="startDate" type="date" className="h-9 text-sm" />
        </div>
        <div className="flex flex-col gap-1.5">
          <Label htmlFor="contract-end" className="text-xs text-[#222222] tracking-[-0.36px]">End date</Label>
          <Input id="contract-end" name="endDate" type="date" className="h-9 text-sm" />
        </div>
      </div>
      <div className="flex flex-wrap gap-5">
        <label className="flex items-center gap-2 text-sm text-[#222222] tracking-[-0.42px] cursor-pointer">
