...
const PatchSchema = z.object({
  ...
  engagementDate: z.string().optional().nullable(),
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ orgId: string; reportId: string }> }) {
  try {
    ...
    const report = await prisma.assuranceEngagement.findUnique({
      where: { id: reportId },
      select: { organizationId: true, status: true, lockedAt: true },
    });
    if (!report || report.organizationId !== orgId) return apiError("NOT_FOUND", "Assurance engagement not found", 404);
    ...
    const updated = await prisma.assuranceEngagement.update({
      ...
      data: {
        ...
        ...(body.engagementDate !== undefined && { engagementDate: body.engagementDate ? new Date(body.engagementDate) : null }),
    ...
    const action = body.status === "issued" ? "assurance_engagement.issued"
      : body.status === "approved" ? "assurance_engagement.approved"
      : body.status === "review" ? "assurance_engagement.submitted_for_review"
      : body.status === "signed_off" ? "assurance_engagement.signed_off"
      : body.status === "superseded" ? "assurance_engagement.superseded"
      : "assurance_engagement.updated";

    await writeAuditLog({
      organizationId: orgId, actorUserId: session.user.id, action: action as any,
      resourceType: "AssuranceEngagement", resourceId: reportId,
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ orgId: string; reportId: string }> }) {
  try {
    ...
    await prisma.assuranceEngagement.delete({ where: { id: reportId } });
    await writeAuditLog({
      organizationId: orgId, actorUserId: session.user.id,
      action: "assurance_engagement.deleted", resourceType: "AssuranceEngagement", resourceId: reportId,
    });
