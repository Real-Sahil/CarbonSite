...
export async function POST(req: NextRequest, { params }: { params: Promise<{ orgId: string; reportId: string }> }) {
  try {
    ...
    const source = await prisma.assuranceEngagement.findUnique({
      where: { id: reportId },
      select: { organizationId: true, status: true, version: true, revisionOf: true, title: true },
    });

    if (!source || source.organizationId !== orgId) return apiError("NOT_FOUND", "Assurance engagement not found", 404);
    if (!["approved", "issued", "signed_off"].includes(source.status)) {
      return apiError("INVALID_STATE", "Can only revise approved or issued engagements", 400);
    }
    ...
    const [newReport] = await prisma.$transaction([
      prisma.assuranceEngagement.create({
        data: {
      ...
      prisma.assuranceEngagement.update({ where: { id: reportId }, data: { status: "superseded" } }),
    ]);

    await writeAuditLog({
      organizationId: orgId, actorUserId: session.user.id, action: "assurance_engagement.revised",
