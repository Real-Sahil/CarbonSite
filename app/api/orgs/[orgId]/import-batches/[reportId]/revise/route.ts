...
export async function POST(req: NextRequest, { params }: { params: Promise<{ orgId: string; reportId: string }> }) {
  try {
    ...
    if (!source || source.organizationId !== orgId) return apiError("NOT_FOUND", "Import batch not found", 404);
    if (!["approved", "issued", "signed_off"].includes(source.status)) {
      return apiError("INVALID_STATE", "Can only revise approved or issued batches", 400);
    }
    ...
    const [newReport] = await prisma.$transaction([
      ...
      prisma.importBatch.update({ where: { id: reportId }, data: { status: "superseded" } }),
    ]);

    await writeAuditLog({
      organizationId: orgId, actorUserId: session.user.id, action: "import_batch.revised",
