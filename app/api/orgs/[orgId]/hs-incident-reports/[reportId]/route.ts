...
// Content only. Status changes go through /api/orgs/[orgId]/hs-incidents/[incidentId] so its guards apply.
export async function PATCH(req: NextRequest, { params }: Params) {
  try {
    ...
    if (isLockedStatus("hs-incident-reports", existing.status)) return lockedError("incident");
    ...
    await writeAuditLog({
      ...
      action: "hs_incident_report.updated",
