...
export async function POST(req: NextRequest, { params }: { params: Promise<{ orgId: string; reportId: string }> }) {
  try {
    ...
    const source = await prisma.environmentalPermit.findUnique({
      where: { id: reportId },
      select: { organizationId: true, status: true, version: true, revisionOf: true, title: true },
    });

    if (!source || source.organizationId !== orgId) {
      return apiError("NOT_FOUND", "Environmental permit not found", 404);
    }

    if (!["approved", "issued", "signed_off"].includes(source.status)) {
      return apiError("INVALID_STATE", "Can only revise approved or issued permits", 400);
    }
    ...
    const [newReport] = await prisma.$transaction([
      prisma.environmentalPermit.create({
        data: {
      ...
      prisma.environmentalPermit.update({
        where: { id: reportId },
    ...
    await writeAuditLog({
      ...
      action: "environmental_permit.revised", resourceType: "EnvironmentalPermit", resourceId: reportId,
