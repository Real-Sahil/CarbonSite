/**
 * API v1.0: Activity Records
 * Offset-based pagination with direct API access.
 * Stable version. Support until: 2025-02-01 (estimated)
 *
 * This is a forwarding route that delegates to the current implementation
 * and adds version headers for backward compatibility.
 */
 ...
 * List activity records for the organization.
 *
 * Query Parameters:
 * - cursor: Pagination cursor (record ID) for next page
 * - periodId: Filter by reporting period ID
 * - categoryId: Filter by emission category ID
 * - reviewStatus: Filter by review status (approved|rejected|pending)
 *
 * Response:
 * {
 *   "data": [...],
 *   "nextCursor": "record-id-123" | null,
 *   "total": 42
 * }
 */
export async function GET(_req: NextRequest, { params }: Params) {
  try {
    ...
    const response = NextResponse.json({ data, nextCursor, total });
    response.headers.set("API-Version", "1.0");
    response.headers.set("X-API-Lifecycle", "stable");
    return response;
  } catch (err) {
...
/**
 * POST /api/v1/orgs/:orgId/activity-records
 ...
 *   "facilityId": "fac-123",
 *   "sourceDescription": "Monthly waste collection"
 * }
 *
