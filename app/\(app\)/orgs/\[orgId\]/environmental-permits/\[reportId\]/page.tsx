...
interface PageProps { params: Promise<{ orgId: string; reportId: string }>; }
...
export default async function DetailPage({ params }: PageProps) {
  ...
  if (!report || report.organizationId !== orgId) return <div className="p-8 text-sm text-red-600">Not found.</div>;
  ...
  return (
    <EnvironmentalPermitEditor
      report={{
        ...
        permitDate: report.permitDate ? report.permitDate.toISOString().slice(0, 10) : null,
        expiryDate: report.expiryDate ? report.expiryDate.toISOString().slice(0, 10) : null,
